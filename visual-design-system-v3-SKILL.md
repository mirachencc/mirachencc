...
**错误做法（单色延伸）：** 把主色加白推导背景 → 整个页面都是同色调深浅，闷、单调、无层次。

**正确做法（主色 + 中性底 + 可选点缀）：**
- 背景和表面色 → 来自**中性色系**（暖白/冷白/浅灰），不从主色延伸
- 主色 → 只用于强调色、标签、边框点缀，面积 ≤ 20%
- 可选点缀色 → 从主色的邻近色或互补色中取一个，用于次级装饰
...
| 主色属性 | 背景中性色方向 | 说明 |
|---------|--------------|------|
| 暖色系（红/橙/黄/暖绿） | 暖白 `#FAFAF7` 或象牙白 `#F8F6F0` | 微暖调，不用主色底 |
| 冷色系（蓝/青/冷绿/紫） | 冷白 `#F8FAFB` 或浅冷灰 `#F5F7FA` | 微冷调，不用主色底 |
| 低饱和/莫兰迪色 | 纯白 `#FFFFFF` + 浅灰表面 `#F5F5F3` | 干净衬托，让主色更显眼 |
| 用户明确要深色 | 深灰 `#1A1A1A` 或纯黑 `#111111` | — |

> **关键：背景色不从主色加白推导，而是从上表直接选取中性色。**

### Step 3：推导完整 9 色 token

```
--color-bg：           从 Step 2 中性底色表选取，不用主色推导
--color-surface：      纯白 #FFFFFF（浅色主题）/ 比 bg 亮 6% （深色主题）
--color-surface-alt：  bg 加深 3%，保持中性，不加主色调
--color-surface-warm： bg 加微暖调（用于强调型区块底色）
--color-border：       bg 加深 10%，中性灰调，不用主色
--color-border-light： bg 加深 5%，更轻
--color-text-primary： 固定 #111111（浅色）/ #F0F0F0（深色）
--color-text-secondary：#555555（浅色）/ #888888（深色）
--color-text-caption： #999999（浅色）/ #666666（深色）
--color-accent：       = 用户提供的主色（直接使用）
--color-accent-bg：    主色加白至 93%，保留 10% 饱和度（只用于标签背景）
```

### Step 4：可选点缀色（提升层次感）

当主色为单一色调时，可引入一个点缀色，用于次级装饰（分割线高亮、图标点缀等）：

| 主色方向 | 推荐点缀色 | 用途 |
|---------|----------|------|
| 绿色系 | 暖金/象牙 `#C8A96E` 或 浅珊瑚 `#E8A598` | 标注色、次级标签 |
| 蓝色系 | 浅橙/金黄 `#E8A020` | 强调数字、高亮标签 |
| 紫色系 | 浅金 `#C9A84C` | 装饰线、图标点缀 |
| 红/橙系 | 深棕 `#6B4C3B` 或冷灰 | 次级文字、边框 |
| 莫兰迪绿 | 暖米 `#F0EBE0` 底 + 浅金点缀 | 背景区块、装饰 |
...
| 主色 | 推荐点缀 | 避免 |
|-----|---------|------|
| 绿色系 | 暖金/象牙/浅珊瑚 | 大面积绿底（沉闷） |
| 蓝色系 | 浅橙/金黄 | 绿色点缀（层次不足） |
| 红/橙系 | 深棕/米白 | 绿色（圣诞感） |
| 紫色系 | 浅金/冷白 | 橙色点缀（刺眼） |
| 莫兰迪系 | 同系邻近色 + 暖米底 | 高饱和点缀（破坏气质） |
